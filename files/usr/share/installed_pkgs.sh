...
while IFS='|' read size_bytes pkg_name pkg_version pkg_time; do
  ...
  # Вычисляем процент от ОБЩЕГО размера overlay (округление до сотых)
  if [ "$overlay_total" -gt 0 ] && [ "$overlay_total" -gt 0 ]; then
    percent=$(awk "BEGIN {printf \"%.2f\", ($size_bytes/1048576 / ($overlay_total/1024)) * 100}")
  else
    percent="N/A"
  fi
  ...
  if [ "$percent" != "N/A" ] && [ "$percent" != "0.00" ]; then
    echo "│ Доля в overlay: ${percent}%"
    
    # Визуальный бар (ширина 50 символов)
    bar_width=50
    filled=$(awk "BEGIN {printf \"%.0f\", ($size_bytes/1048576 / ($overlay_total/1024)) * $bar_width}")
...
if [ "$overlay_total" -gt 0 ]; then
  total_percent=$(awk "BEGIN {printf \"%.2f\", ($total_bytes/1048576 / ($overlay_total/1024)) * 100}")
  echo "  └─ Занимают от общего размера overlay: ${total_percent}%"
fi
...
if [ -n "$total_percent" ] && [ $(echo "$total_percent > 80" | bc) -eq 1 ]; then
  echo ""
  echo "⚠️  ВНИМАНИЕ: Ваши пакеты занимают более 80% всего overlay раздела!"
